...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if let masterController = navController.topViewController as? MasterViewController{
            
            masterController.draws = draws
        }
    ...
    func saveData() {
        ...
        if let encodedData = try? JSONEncoder().encode(draws) {
    ...
    func loadData() {
        ...
        if let jsondata = FileManager.default.contents(atPath: fileURL.path) {
            ...
            do {
                draws = try decoder.decode(Draws.self, from: jsondata)
            } catch {
                fatalError(error.localizedDescription)
            }
        } else {
